...
struct StepperDopr5 : StepperBase{
	...
	VecDoub dydxnew;
	StepperDopr5(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atoll, const Doub rtoll, bool dens);
...
template <class D>
StepperDopr5<D>::StepperDopr5(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atoll, const Doub rtoll, bool dens):
StepperBase(yy,dydxx,xx,atoll,rtoll,dens), k2(n), k3(n), k4(n), k5(n), k6(n), rcont1(n), rcont2(n), rcont3(n), rcont4(n), rcont5(n), dydxnew(n){
...
void StepperDopr5<D>::dy(const Doub h, D &derivs){
	static const Doub c2=0.2, c3=0.3, c4=0.8, c5=8.0/9.0, a21=0.2, 
	a31=3.0/40.0, a32=9.0/40.0, a41=44.0/45.0, a42=-56.0/15.0,a43=32.0/9.0, a51=19372.0/6561.0, a52=-25360.0/2187.0, 
	a53=64448.0/6561.0, a54=-212.0/729.0, a61=9017.0/3168.0, a62=-355.0/33.0, a63=46732.0/5247.0, a64=49.0/176.0, 
	a65=-5103.0/18656.0, a71=35.0/384.0, a73=500.0/1113.0, a74=125.0/192.0, a75=-2187.0/6784.0, a76=11.0/84.0, 
	e1=71.0/57600.0, e3=-71.0/16695.0, e4=71.0/1920.0, e5=-17253.0/339200.0, e6=22.0/525.0, e7=-1.0/40.0;
	VecDoub ytemp(n);
	Int i;
	for (i=0;i<n;i++)                  //step 1
		ytemp[i]=y[i]+h*a21*dydx[i];
	derivs(x+c2*h, ytemp, k2);         //step 2
	for (i=0;i<n;i++)      
		ytemp[i]=y[i]+h*(a31*dydx[i] + a32*k2[i]);
	derivs(x+c3*h, ytemp, k3);         //step 3
	for (i=0;i<n;i++)                  
		ytemp[i]=y[i]+h*(a41*dydx[i] + a42*k2[i] + a43*k3[i]);
	derivs(x+c4*h, ytemp, k4);         //step 4
	for (i=0;i<n;i++)                  
		ytemp[i]=y[i]+h*(a51*dydx[i] + a52*k2[i] + a53*k3[i] + a54*k4[i]);
	derivs(x+c5*h, ytemp, k5);         //step 5
	for (i=0;i<n;i++)                  
		ytemp[i]=y[i]+h*(a61*dydx[i] + a62*k2[i] + a63*k3[i] + a64*k4[i] + a65*k5[i]);
	Doub xph=x+h;
	derivs(xph,ytemp,k6);              //step 6
	for (i=0;i<n;i++)                  
		yout[i]=y[i]+h*(a71*dydx[i] + a73*k3[i] + a74*k4[i] + a75*k5[i] + a76*k6[i]);
	derivs(xph,yout,dydxnew);
	for (i=0;i<n;i++) {  // error estimate substracts 4th and 5th order methods
...
void StepperDopr5<D>::prepare_dense(const Doub h, D &derivs){
	VecDoub ytemp(n);
	static const Doub d1=-12715105075.0/11282082432.0, d3=87487479700.0/32700410799.0, d4=-10690763975.0/1880347072.0,
	d5=701980252875.0/199316789632.0, d6=-1453857185.0/822651844.0, d7=69997945.0/29380423.0;
	for (Int i=0;i<n;i++) {
		rcont1[i]=y[i];
...
bool StepperDopr5<D>::Controller::success(const Doub err, Doub &h){
	...
	//Must set beta to nonzero value for PI control (NR suggests beta=0.04-0.08)
	Doub scale;
	if (err <= 1.0){
		if (err == 0.0)
			scale=maxscale;
